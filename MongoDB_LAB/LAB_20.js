...
db.Deposit.insertOne({ACTNO:109,CNAME:"KIRTI",BNAME:"VIRAR",AMOUNT:3000,ADATE:new Date("3-5-97")})
...
// 7. Display all documents of "Student" collection whose age is equal to 25 or 30 or 35. (using $or as well as using 
// $in). 
...
db.Student.find({$and:[{age:30},{name:"John"}]})
...
db.Student.find({$and:[{name:"John"},{city:"New York"}]},{name:1,age:1,_id:0})


// Part – C 

// 1. Display name of students from "Student" collection whose age is between to 25 and 35 and sort output by 
// age in ascending order. 
db.Student.find({$and:[{age:{$gte:25}},{age:{$lte:35}}]},{name:1,age:1,_id:0}).sort({age:1})

// 2. Display all documents of "Student" collection and sort all the documents by name in ascending order and then 
// by age in descending. 
...
// 6. Display all documents of "Student" collection in such a way that skip the first 2 documents and return the rest 
// documents. 
